...
export const refreshAccessToken = async ({ force = false } = {}) => {
    ...
    if (
        !force &&
        existingToken &&
        !isAccessTokenExpiredOrNearExpiry(existingToken)
    ) {
        return existingToken;
    }

    // Important: only one refresh request may be in flight.
    // This prevents refresh-token rotation from invalidating itself
    // when multiple pages/components request a refresh together.
    if (refreshPromise) {
...
export const initializeAuthSession = async () => {
    ...
    // No local session. A protected page will decide whether to redirect.
    if (!token) {
        return null;
    }
...
export const apiFetch = async (url, options = {}) => {
    ...
    // Refresh before making a request when the JWT is already expired
    // or close to expiry.
    ...
    // The server rejected the access token. Refresh once and retry once.
    token = await refreshAccessToken({ force: true });
